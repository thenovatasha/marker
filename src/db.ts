 ...
 */
export async function saveMark(markTag: string, filePath: string, replace: boolean) {
...
// 
export async function deleteMark(markTag: string) {
    let db = await getDatabase();
    const index = findIndex(markTag, db);
    if (index === MISSING) {
        return "";
    ...
    let path: string = "[could not read file]";
    try {
        await db.update((marks) => {
...
//
// }
export async function listAll(): Promise<Mark[]> {
    let db = await getDatabase();
    return db.data.marks;
}
function findIndex(markTag: string, db: Low<MarkCollection>) {
    return db.data.marks.findIndex((mark) => mark.mark === markTag);
}

export async function getMark(markTag: string) {
